 ...
 * ------------------------------------------------------
 * WotMedia - Modified to fit the wordpress plugin
 * ------------------------------------------------------   
 */
...
(function() {
	...
	tinymce.create('tinymce.plugins.Layer', {
		...
		_visualAid : function(ed, e, s) {
			...
			tinymce.each(dom.select('ul,h1,h2,h3,h4,h5,h6,span,img', e), function(e) {
		...
		_move : function(d) {
			var ed = this.editor, i = 0, z = [], ci = -1, fi = -1, nl;

      if ( wtl = ed.dom.get('mceWotlayer') ) {
          le = wtl;    
          
         if ( le.id == 'close' ) return false;
         ...
         {           
           //no < zero index
           d = ( d == 0 ) ? 1 : d;           
           dx = ( d == 1 ) ? parseInt(zi) + 1 : parseInt(zi) - 1;
         ...
         ed.dom.add( le, 'div', {id : 'zindex'}, 'Zindex : ' + zi  );
